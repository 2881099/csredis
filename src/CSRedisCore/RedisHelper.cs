...
public abstract partial class RedisHelper {
	...
	public bool GeoAdd(string key, double longitude, double latitude, object member) => Instance.GeoAdd(key, longitude, latitude, member);
	/// <summary>
	/// 将指定的地理空间位置（纬度、经度、成员）添加到指定的key中。这些数据将会存储到sorted set这样的目的是为了方便使用GEORADIUS或者GEORADIUSBYMEMBER命令对数据进行半径查询等操作。
	/// </summary>
	/// <param name="key">不含prefix前辍</param>
	...
	public long GeoAdd(string key, params (double longitude, double latitude, object member)[] values) => Instance.GeoAdd(key, values);
	/// <summary>
	...
	public double? GeoDist(string key, object member1, object member2, GeoUnit unit = GeoUnit.m) => Instance.GeoDist(key, member1, member2, unit);
	/// <summary>
	...
	public string[] GeoHash(string key, object[] members) => Instance.GeoHash(key, members);
	/// <summary>
	...
	public (double longitude, double latitude)?[] GeoPos(string key, object[] members) => Instance.GeoPos(key, members);
	...
	/// <returns></returns>
	public static CSRedisClientLock Lock(string name, int timeoutSeconds) => Instance.Lock(name, timeoutSeconds);
}